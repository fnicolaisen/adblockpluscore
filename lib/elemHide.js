...
class ElemHide
{
  ...
  add(filter)
  {
    ...
    let {text, domains, selector} = filter;
  ...
  remove(filter)
  {
    ...
    let {text, selector} = filter;

    // Unconditially applied element hiding filters
    if (this._filterBySelector.get(selector) == text)
    {
    ...
    {
      this._filtersByDomain.remove(text, filter.domains);
    }

    this._filters.delete(text);
    filterNotifier.emit("elemhideupdate");
  }
    ...
    for (let currentDomain of domainSuffixes(domain, !specificOnly))
    {
      ...
      if (map)
      {
                                         ...
                                         map.entries() : [[map, true]]))
        {
          ...
          {
            let {selector} = Filter.fromText(filter);
  ...
  _onExceptionAdded(exception)
  {
    ...
    {
      let filter = Filter.fromText(unconditionalFilterForSelector);
