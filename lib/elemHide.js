...
function parseFilter(text)
{
  ...
  return {
    text,
    selector: text.substring(index + 2),
    domains: text.substring(0, index)
  };
}
...
class ElemHide
{
  ...
  constructor()
  {
     ...
     *
     * This is based on the value of the keys of
    ...
    /**
     * Whether `{@link module:elemHide~ElemHide#getStyleSheet getStyleSheet()}`
     * is ready to do filter matching.
     *
     * @type {boolean}
     *
     * @private
     */
    this._readyForMatching = false;
                ...
                includeExceptions = false)
  {
    ...
    if (!this._readyForMatching)
      this._prepareForMatching();
  ...
  /**
   * Clears the internal data structures used by
   * `{@link module:elemHide~ElemHide#getStyleSheet getStyleSheet()}` for
   * filter matching.
   *
   * @private
   */
  ...
  /**
   * Prepares `{@link module:elemHide~ElemHide#getStyleSheet getStyleSheet()}`
   * for filter matching.
   *
   * @private
   */
  _prepareForMatching()
  {
    ...
    for (let text of this._filters)
    {
      let {domains, selector} = parseFilter(text);

      domains = domains == "" ? null : parseDomains(domains, ",");

      if (!(domains || elemHideExceptions.hasExceptions(selector)))
      {
        // The new filter's selector is unconditionally applied to all domains
        this._filterBySelector.set(selector, text);
      }
      else
      {
        // The new filter's selector only applies to some domains
        this._filtersByDomain.add(text, domains);
      }
    }

    this._readyForMatching = true;
  }
  ...
  {
    return [...this._filterBySelector.keys()];
  }
  ...
  _getConditionalSelectors(domain, specificOnly)
  {
    ...
    for (let currentDomain of domainSuffixes(domain, !specificOnly))
    {
      ...
      if (map)
      {
        for (let [filter, include] of (typeof map != "string" ?
                                         map.entries() : [[map, true]]))
        {
          ...
          {
            let {selector} = parseFilter(filter);
            if (!elemHideExceptions.getException(selector, domain))
