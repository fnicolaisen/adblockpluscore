...
class FilterListener
{
      ...
      for (let subscription of filterStorage.subscriptions())
      {
        if (shouldDeployFilters(subscription))
        {
          promise = promise.then(() =>
          {
            ...
            {
              deployFilter(this._engine, Filter.fromText(text, false),
                           [subscription]);
            }
          });
        }
  ...
  _onSubscriptionRemoved(subscription)
  {
    ...
    {
      for (let text of subscription.filterText())
        undeployFilter(this._engine, Filter.fromText(text, false));
    }
  }
  ...
  _onFilterStateEnabled(text, newValue)
  {
    ...
    if (newValue == false)
      undeployFilter(this._engine, Filter.fromText(text, true));
    else
      deployFilter(this._engine, Filter.fromText(text, true));
  }
