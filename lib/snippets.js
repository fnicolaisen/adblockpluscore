...
class Snippets extends EventEmitter
{
  ...
  clear()
  {
    let {_filters, _filtersByDomain} = this;

    if (_filters.size == 0)
      return;

    _filters.clear();
    _filtersByDomain.clear();
  ...
  add(filter)
  {
    ...
    _filters.add(filter.text);

    if (size != _filters.size)
    {
      _filtersByDomain.add(filter.text, filter.domains);
  ...
  remove(filter)
  {
    ...
    _filters.delete(filter.text);

    if (size != _filters.size)
    {
      _filtersByDomain.remove(filter.text, filter.domains);
      ...
      if (map)
      {
                                       ...
                                       map.entries() : [[map, true]]))
        {
          if (!include)
            ...
            filters.push(Filter.fromText(text));
        }
      }
    }
 ...
 */
exports.compileScript = function compileScript(script,
                                               libraries,
                                               environment = {})
{
  return `
    "use strict";
    {
      ...
      let environment = JSON.stringify(${JSON.stringify(environment)});

      let script = ${JSON.stringify(parseScript(script))};
