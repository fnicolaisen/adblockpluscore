 ...
 *         Array.<module:filterClasses~URLFilter>} filters The filters. If the
...
exports.CombinedMatcher = class CombinedMatcher
{
  constructor()
  {
    ...
    /**
     * Lookup table of previous search results
     * @type {module:caching.Cache.<string, ?MatcherSearchResults>}
     * @private
     */
    this._searchResultCache = new Cache(1000);
  ...
  match(url, typeMask, docDomain, sitekey, specificOnly)
  {
    let key = url.href + " " + typeMask + " " + docDomain + " " +
              (sitekey || "") + " " + (specificOnly ? "1" : "");

    let cache = this._matchResultCache;
    if (!cache)
    {
      cache = new Array(64).fill(null).map(() => ({key: null, value: null}));
      this._matchResultCache = cache;
    }

    let entry = cache[key.length & 63];
    if (entry.key == key)
      return entry.value;

    let result = this._matchInternal(url, typeMask, docDomain, sitekey,
                                     specificOnly);
  ...
  search(url, typeMask, docDomain, sitekey, specificOnly, filterType = "all")
  {
    ...
    let result = this._searchResultCache.get(key);
    ...
    this._searchResultCache.set(key, result);
