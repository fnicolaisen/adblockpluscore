...
exports.FilterMap = class FilterMap
{
   ...
   *   string or a `{@link module:filterClasses.Filter}` and a boolean value.
   */
  entries()
  {
   ...
   *
   * @returns {object} An iterator that yields a string or a
   *   `{@link module:filterClasses.Filter}`.
   */
  keys()
  {
   ...
   *
   * @param {string|module:filterClasses.Filter} key The filter.
   *
...
exports.FiltersByDomain = class FiltersByDomain
{
   ...
   *
   * @returns {object} An iterator that yields a two-tuple containing a
   *   string or a `{@link module:filterClasses.Filter}` and either a
   *   `{@link module:filtersByDomain.FilterMap FilterMap}` object
   *   or a single string or a single `{@link module:filterClasses.Filter}`. In
   *   the latter case, it directly indicates that the filter should be
   *   applied.
   */
  entries()
  {
  ...
  /**
   * @typedef
   *   {module:filtersByDomain.FilterMap|string|module:filterClasses.Filter}
   *   FiltersByDomainValue
   */
   ...
   * @param {string} key The domain.
   *
   * @returns {module:filtersByDomain~FiltersByDomainValue|undefined}
   *   Either a `{@link module:filtersByDomain.FilterMap FilterMap}` object or
   *   a single string or a `{@link module:filterClasses.Filter}`. In the
   *   latter case, it directly indicates that the filter should be applied. If
   *   this `FiltersByDomain` object does not contain the domain referenced by
   ...
   *
   * @param {string|module:filterClasses.Filter} filter The filter.
  ...
  add(filter, domains)
  {
    for (let [domain, include] of domains || defaultDomains)
    {
      ...
      if (!map)
      {
        this._map.set(domain, include ? filter : new FilterMap([[filter,
                                                                 false]]));
      }
      else if (!(map instanceof FilterMap))
      {
        if (filter != map)
        {
